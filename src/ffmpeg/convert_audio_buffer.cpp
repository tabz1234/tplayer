...
std::vector<FFmpeg::Frame<FFmpeg::MediaType::audio>> FFmpeg::convert_audio_buffer_format(
    const std::vector<Frame<MediaType::audio>>::const_iterator begin,
    ...
    std::for_each(begin, end, [&](const auto& cur_frame) {
        Frame<MediaType::audio> resampled_frame;

        resampled_frame.ptr()->sample_rate = cur_frame.ptr()->sample_rate;
        resampled_frame.ptr()->channel_layout = cur_frame.ptr()->channel_layout;
        resampled_frame.ptr()->format = new_format;
        resampled_frame.ptr()->nb_samples = cur_frame.ptr()->nb_samples;

        av_frame_copy_props(resampled_frame.ptr(), cur_frame.ptr());

        c_api_ret = swr_convert_frame(swr_ctx_, resampled_frame.ptr(), cur_frame.ptr());
